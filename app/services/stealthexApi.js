...
exports.moneroTransfer = async (toAddress, amount) => {
  ...
  const daemon = await monerojs.connectToDaemonRpc('https://testnet.xmr.ditatompel.com:443', 'superuser', 'abctesting123');
  let walletRpc = await monerojs.connectToWalletRpc("http://localhost:28084", "rpc_user", "abc123");// Connect to a Monero daemon

  // Get transaction by transaction ID
  const txId = 'af07bbfed04185502fb28e241bbaf63c62385dac484cf901e774bc35cf7ada7f';
  ...
  const confirms = currentHeight - tx.getBlock().getHeader().getHeight() + 1;
  ...
  utils.handleSuccess(res, 201, { primaryAddress, mnemonic, balance, createdTx })
